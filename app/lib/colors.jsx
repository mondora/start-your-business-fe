// Common styles
export const background = '#ffffff';
export const backgroundLightGrey = '#eaeaed';
export const buttonPrimary = '#20bda9';
export const primaryColor = '#20bda9';
export const primaryColorLighter = '#cdd2dc';
export const secondaryColor = '#f7931e';
export const transparent = 'transparent';
export const white = '#ffffff';
export const lightGrey = '#d8d8d8';
export const grey = '#999999';
export const darkGrey = '#38424e';
export const black = '#000000';
export const blackOpacity2 = 'rgba(0, 0, 0, 0.5)';
// Error
export const backgroundError = '#ff1744';
export const backgroundIconError = '#c41637';
// Footer
export const backgroundFooter = '#171b23';
export const backgroundBottomFooter = '#0c0e11';
